...
class TestMetacriticStrategies:
    """Tests for Metacritic extraction strategies"""

    @pytest.fixture
    def strategies(self):
        """Get the Metacritic strategies"""
        return metacritic_extraction_strategies("https://metacritic.com")

    def test_main_image_strategy(self, strategies):
        """Test the main image strategy"""
        main_image_strategy = strategies[0]
        
        # Test with main image present
        html = """
        <html><body>
            <img class="product_image" src="https://metacritic.com/image.jpg">
        </body></html>
        """
        soup = BeautifulSoup(html, "html.parser")
        result = main_image_strategy(soup, html)
        assert result == "https://metacritic.com/image.jpg"
        
        # Test with no main image
    ...
    def test_og_image_strategy(self, strategies):
        ...
        html = """
        <html><head>
            <meta property="og:image" content="https://metacritic.com/og-image.jpg">
        </head></html>
        """
        soup = BeautifulSoup(html, "html.parser")
        result = og_image_strategy(soup, html)
        assert result == "https://metacritic.com/og-image.jpg"
    ...
    def test_large_image_strategy(self, strategies):
        """Test the large image strategy"""
        large_image_strategy = strategies[2]
        
        # Test with large image present
        html = """
        <html><body>
            <img class="large_album_cover" src="https://metacritic.com/large.jpg">
        </body></html>
        """
        soup = BeautifulSoup(html, "html.parser")
        result = large_image_strategy(soup, html)
        assert result == "https://metacritic.com/large.jpg"
        
        # Test with no large image
        html = "<html><body>No image</body></html>"
        soup = BeautifulSoup(html, "html.parser")
        result = large_image_strategy(soup, html)
        assert result is None

    def test_schema_image_strategy(self, strategies):
        """Test the schema.org image strategy"""
        schema_image_strategy = strategies[3]
        
        # Test with schema image present
        html = """
        <html><body>
            <img itemprop="image" src="https://metacritic.com/schema.jpg">
        </body></html>
        """
        soup = BeautifulSoup(html, "html.parser")
        result = schema_image_strategy(soup, html)
        assert result == "https://metacritic.com/schema.jpg"
        
        # Test with no schema image
        html = "<html><body>No image</body></html>"
        soup = BeautifulSoup(html, "html.parser")
        result = schema_image_strategy(soup, html)
        assert result is None

    def test_header_image_strategy(self, strategies):
        """Test the header image strategy"""
        header_image_strategy = strategies[4]
        
        # Test with header image present
        html = """
        <html><body>
            <div class="product_page_title_row">
                <img src="https://metacritic.com/header.jpg">
            </div>
        </body></html>
        """
        soup = BeautifulSoup(html, "html.parser")
        result = header_image_strategy(soup, html)
        assert result == "https://metacritic.com/header.jpg"
        
        # Test with no header image
        html = "<html><body><div class='product_page_title_row'></div></body></html>"
        soup = BeautifulSoup(html, "html.parser")
        result = header_image_strategy(soup, html)
        assert result is None
        
        # Test with no header element
        html = "<html><body>No header</body></html>"
        soup = BeautifulSoup(html, "html.parser")
        result = header_image_strategy(soup, html)
        assert result is None
