...
class ScoreStore(Store):
    ...
    def get_all(self):
        ...
        for score_id, score in scores.items():
            ...
            if isinstance(release, dict):
                ...
                if "cover_url" in release and release["cover_url"]:
                    ...
                    print(
                        f"Added cover URL from release dict to score {score_id}: {release['cover_url']}"
                    )
                ...
                if hasattr(release, "cover_url") and release.cover_url:
                    score["cover_url"] = release.cover_url
                    print(
                        f"Added cover URL from release object to score {score_id}: {release.cover_url}"
                    )
                    scores_with_covers += 1
                else:
                    scores_without_covers += 1
        ...
        print(
            f"Cover URL Statistics: {scores_with_covers}/{total_scores} scores have cover URLs ({scores_with_covers/total_scores*100:.1f}%)"
        )
        ...
        filtered_scores = {
            ...
            if value.get("reviews_counted") > MINIMUM_REVIEWS_COUNTED
        }
        print(
            f"Returning {len(filtered_scores)} scores after filtering by minimum reviews"
        )
