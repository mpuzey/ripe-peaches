"""
Utility for prefetching and caching album cover URLs (AOTY only)
"""
import json
import os
import time
...
def get_cached_cover_url(url: str, source: str = "aoty") -> Optional[str]:
    """
    Get a cached cover URL if it exists
    
    Args:
        url: Album URL to look up
        source: Source site ("aoty")
        
    Returns:
        Cached cover URL or None if not found
    """
    cache_file = AOTY_CACHE_FILE
    cache = load_cache(cache_file)
    return cache.get(url)

if __name__ == "__main__":
    # Example usage for prefetching
    from constants import AOTY_REQUEST_HEADERS
