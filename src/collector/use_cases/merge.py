...
def merge_artist_dicts(
    archive: Dict[str, Artist], new_artist_entries: Dict[str, Artist]
) -> Dict[str, Artist]:
    ...
    for artist_id, recently_reviewed_artist in new_artist_entries.items():
        if artist_id in merged_archive:
            merged_archive = update_artist_entry(
                merged_archive, recently_reviewed_artist
            )
        else:
...
def update_artist_entry(
    archive: Dict[str, Artist], new_artist_entry: Artist
) -> Dict[str, Artist]:
    ...
    if not _has_valid_releases(archived_artist):
        ...
        merged_releases = merge_releases(
            archived_artist.releases, new_artist_entry.releases
        )
...
def merge_releases(
    archived_releases: List[Release], new_release_entries: List[Release]
) -> List[Release]:
    # Create a dictionary of existing releases by ID for faster lookup
    archived_releases_by_id = {
        release.id: release for release in archived_releases if release
    }
    result_releases = []
...
def _process_existing_releases(
    archived_releases, new_release_entries, processed_new_releases
):
    ...
    for archived_release in archived_releases:
        ...
        matching_new_releases = [
            r for r in new_release_entries if r.id == archived_release.id
        ]
...
def _create_merged_release(archived_release, new_release):
    ...
    merged_release.reviews = merge_review_lists(
        archived_release.reviews, new_release.reviews
    )
...
def _add_new_releases(
    ...
    for new_release in new_release_entries:
        if (
            new_release.id not in processed_new_releases
            and new_release.id not in archived_releases_by_id
        ):
...
def merge_review_lists(
    archived_reviews: List[Review], recent_reviews: List[Review]
) -> List[Review]:
