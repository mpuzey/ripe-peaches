...
class Spotify:
    ...
    def __init__(self, session):
        print(
            f"Initializing Spotify with Client ID length: {len(SPOTIFY_CLIENT_ID) if SPOTIFY_CLIENT_ID else 'None'}, Secret length: {len(SPOTIFY_CLIENT_SECRET) if SPOTIFY_CLIENT_SECRET else 'None'}"
        )
    ...
    def _request_access_token(encoded):
        try:
            ...
            if resp.status_code != 200:
                print(
                    f"Spotify authentication failed with status code: {resp.status_code}"
                )
    ...
    async def get_album(self, artist_name, album_name):
        ...
        if not self.access_token:
            ...
            return {}
        try:
            search_result = await self.search_by_album_and_artist(
    ...
    def _find_best_album_id(self, search_result, artist_name, album_name):
        items = (
            search_result.get("albums", {}).get("items", []) if search_result else []
        )
        for album in items:
            if (
                album.get("name", "").lower() == album_name.lower()
                and any(
                    a["name"].lower() == artist_name.lower()
                    for a in album.get("artists", [])
                )
                and album.get("album_type") == "album"
            ):
                return album["id"]
        return None
    ...
    async def get_album_details(self, album_id):
        ...
        async with self.session.get(
            ...
            if response.status != 200:
                print(
                    f"Error fetching album details: {response.status} - {response.reason}"
                )
                return None
            try:
    ...
    async def search_by_album(self, artist_name, album_name):
        ...
        async with self.session.get(
            ...
            return await self._parse_search_response(
                response, artist_name, album_name, by_artist=False
            )

    async def _parse_search_response(
        self, response, artist_name, album_name, by_artist=True
    ):
        result = {"albums": {"items": []}}
        ...
        try:
            ...
            if by_artist:
                ...
                filtered = [
                    ...
                    for a in items
                    if any(
                        artist_name.lower() in artist["name"].lower()
                        for artist in a["artists"]
                    )
                    and a["name"].lower() == album_name.lower()
                ]
    ...
    def _fetch_releases_for_char(self, char):
        ...
        while next_page:
            offset = dict(parse.parse_qsl(parse.urlsplit(next_page).query)).get(
                "offset", 0
            )
            if int(offset) >= 10000:
                break
            ...
            response = requests.get(
                next_page, headers={"Authorization": f"Bearer {self.access_token}"}
            ).json()
